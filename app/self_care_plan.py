    ...
    def get_input(prompt, valid_options):
        while True:
            ...
            if response in valid_options:
                return response
            print(
                f"Please enter one of the following options: {', '.join(valid_options)}"
            )

    sleep_quality = get_input(
        "How would you rate your sleep quality? (poor/average/good): ",
        ["poor", "average", "good"],
    )
    diet = get_input(
        "How healthy is your diet? (poor/average/good): ", ["poor", "average", "good"]
    )
    physical_activity = get_input(
        "How often do you exercise? (none/1-2 times per week/3+ times per week): ",
        ["none", "1-2 times per week", "3+ times per week"],
    )
    social_connection = get_input(
        "How connected do you feel with friends or family? (not connected/somewhat connected/very connected): ",
        ["not connected", "somewhat connected", "very connected"],
    )
